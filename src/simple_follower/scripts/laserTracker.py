...
class laserTracker:
	...
	def registerScan(self, scan_data):
		...
		# filter invalid data
		ranges[np.isnan(ranges) | np.isinf(ranges)] = scan_data.range_max
		ranges[(ranges < scan_data.range_min) | (ranges > scan_data.range_max)] = scan_data.range_max
		...
		if(minDistance > scan_data.range_max):
			...
			rospy.logwarn('laser no object found, minDistance: %f, scan_data.range_max: %f', minDistance, scan_data.range_max)
