...
- `/camera/depth_to_color_overlay/image_raw`: 对齐深度的人眼可视化 RGB 叠加图。
...
- `enable_d2c_viewer`: 发布对齐到彩色坐标系的深度图像到 `depth_to_color/image_raw`，以及人眼可视化叠加图到 `depth_to_color_overlay/image_raw`。
